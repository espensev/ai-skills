...
not the live routing skill. Use `/delegate` for one bounded task; use
`/delegation-eval` to decide which local models and task mappings are actually
...
|---------|-------|---------|
| `status` | `/delegation-eval` or `/delegation-eval status` | Inspect telemetry repo, API health, recent eval runs, and dispatch-rule drift. |
| `run` | `/delegation-eval run [pack] [candidates]` | Run a controller-assistant or no-interference eval pack. |
| `judge` | `/delegation-eval judge <run-id>` | Export a judge packet, score it with the controller, and apply judgments. |
| `recommend` | `/delegation-eval recommend` | Aggregate judged evals into task_type -> model recommendations. |
| `update-rules` | `/delegation-eval update-rules` | Propose a reviewed dispatch-rules.json patch from recommendations. |
