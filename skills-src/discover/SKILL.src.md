...
```
{{cmd}}discover <what you need to know>
```

Examples:

```
{{cmd}}discover Map the import graph and find circular dependencies
{{cmd}}discover What would break if we split collector.py into three files?
{{cmd}}discover Where are all the places that touch the sessions table?
{{cmd}}discover Is it feasible to replace HTTP transport with WebSocket?
{{cmd}}discover What test coverage exists for the pricing module?
```
