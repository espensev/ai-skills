...
# Build Gate {{dash}} Multi-Target Build Validator

Validates the project's build chain across one or more configured targets.
Reads target definitions from `[build-gate.<name>]` tables in project.toml {{dash}}
each target declares its own build, verify, and test commands.

**All commands run to completion autonomously.**

**Config:** `.{{provider-lc}}/skills/project.toml` {{dash}} targets defined in `[build-gate.<name>]`
...
|---------|-------|---------|
| `verify` | `{{cmd}}build-gate` or `{{cmd}}build-gate verify` | Check current artifact state across all targets |
| `plan` | `{{cmd}}build-gate plan` | Verify first, then report what needs building/rebuilding and why |
| `test` | `{{cmd}}build-gate test [target]` | Run tests for one or all targets |
| `build` | `{{cmd}}build-gate build [target]` | Build one or all targets, then verify |
| `all` | `{{cmd}}build-gate all` | Full pipeline: verify {{arrow}} plan {{arrow}} build stale/missing {{arrow}} test {{arrow}} final verify |
...
3. Enumerate all `[build-gate.<name>]` tables {{dash}} these are the targets
...
## Command: `verify` {{dash}} What's the Current State?
...
1. For each target that has `artifacts` or `verify` configured:
   a. Check each artifact path {{dash}} exists? non-zero bytes?
   b. Check file age {{dash}} fresh (< 1h), ok (< 24h), or stale (> 24h)
   c. Run the `verify` command if configured
   d. Run the `drift` command if configured {{dash}} flag source-list mismatches
2. For targets with no artifacts/verify, mark as UNKNOWN (can't verify)

### Report

```
Build Gate {{dash}} Verify

  Target      Artifacts    Freshness    Drift    Status
  backend     2/2          2h ago       clean    PASS
  frontend    1/2 missing  {{dash}}            {{dash}}        FAIL
  py-tests    (none)       {{dash}}            {{dash}}        UNKNOWN
...
## Command: `plan` {{dash}} What Needs Work?
...
3. Resolve `depends` ordering {{dash}} if target A depends on B and B needs
...
```
Build Gate {{dash}} Plan

  Targets needing work:
  1. backend    {{dash}} DRIFT (source list changed since last build)
  2. frontend   {{dash}} FAIL  (dist/assets/ missing, depends on backend)

  Build order: backend {{arrow}} frontend
  Estimated: 2 targets, ~40s based on prior runs

  Clean targets:
  - py-tests   {{dash}} PASS (156/156 tests, 4s ago)
```

---

## Command: `test` {{dash}} Run Tests

Run test commands for one or all targets.

### Usage

- `{{cmd}}build-gate test` {{dash}} run tests for all targets that have `test` configured
- `{{cmd}}build-gate test backend` {{dash}} run tests for one target only
...
## Command: `build` {{dash}} Build Targets

Build one or all targets, then verify.

### Usage

- `{{cmd}}build-gate build` {{dash}} build all targets in dependency order
- `{{cmd}}build-gate build frontend` {{dash}} build one target only
...
## Command: `all` {{dash}} Full Pipeline
...
1. **Verify** {{dash}} assess current artifact state
2. **Plan** {{dash}} identify what needs work
3. **Build** {{dash}} rebuild only targets that need it (FAIL, STALE, DRIFT),
   in dependency order. Skip targets already PASS.
4. **Test** {{dash}} run all target tests + global test command
5. **Final verify** {{dash}} confirm all artifacts are now fresh and valid
...
- **Build failure**: Capture full stderr, identify the first error line, report
  ...
  dependency {{dash}} mark dependents as BLOCKED).
...
|-------|---------------------|
| `{{cmd}}manager verify` | Delegate to `{{cmd}}build-gate all` for comprehensive build validation |
| `{{cmd}}manager merge` | Run `{{cmd}}build-gate verify` after merge to confirm no artifact corruption |
| `{{cmd}}ship` | Run `{{cmd}}build-gate verify` before commit {{dash}} if FAIL, suggest `{{cmd}}build-gate all` |
| `{{cmd}}qa` | `{{cmd}}build-gate test` covers build-scoped tests; `{{cmd}}qa` covers full test suite |
| `{{cmd}}observer` | Record `build-error` observations when targets fail |

---

## Conventions

- Never delete build output {{dash}} only verify it exists
...
- Respect `depends` ordering {{dash}} never build a target before its dependencies
- When build commands use platform-specific tooling (PowerShell, Make, etc.),
  invoke them appropriately for the current platform
- Default command is `verify`, not `build` {{dash}} check before acting
