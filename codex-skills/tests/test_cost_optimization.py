...
class TestResolveModelForTask(unittest.TestCase):
    ...
    def test_default_mapping_low(self):
        self.assertEqual(resolve_model_for_task({"complexity": "low"}, {}), "mini")

    def test_default_mapping_medium(self):
        self.assertEqual(resolve_model_for_task({"complexity": "medium"}, {}), "standard")

    def test_default_mapping_high(self):
        self.assertEqual(resolve_model_for_task({"complexity": "high"}, {}), "max")

    def test_missing_complexity_defaults_to_mini(self):
        self.assertEqual(resolve_model_for_task({}, {}), "mini")

    def test_empty_complexity_defaults_to_mini(self):
        self.assertEqual(resolve_model_for_task({"complexity": ""}, {}), "mini")

    def test_none_complexity_defaults_to_mini(self):
        self.assertEqual(resolve_model_for_task({"complexity": None}, {}), "mini")

    def test_config_override(self):
        cfg = {"models": {"low": "standard", "medium": "max", "high": "max"}}
        self.assertEqual(resolve_model_for_task({"complexity": "low"}, cfg), "standard")
        self.assertEqual(resolve_model_for_task({"complexity": "medium"}, cfg), "max")

    def test_config_partial_override_falls_through(self):
        cfg = {"models": {"high": "standard"}}
        # high overridden, low uses default
        self.assertEqual(resolve_model_for_task({"complexity": "high"}, cfg), "standard")
        self.assertEqual(resolve_model_for_task({"complexity": "low"}, cfg), "mini")

    def test_invalid_model_in_config_falls_back_to_standard(self):
        cfg = {"models": {"low": "gpt-4"}}
        self.assertEqual(resolve_model_for_task({"complexity": "low"}, cfg), "standard")

    def test_unknown_complexity_value_defaults_to_mini(self):
        self.assertEqual(resolve_model_for_task({"complexity": "extreme"}, {}), "mini")
    ...
    def test_valid_models_set(self):
        self.assertEqual(_VALID_MODELS, {"mini", "standard", "max"})
...
class TestBuildTelemetryPayload(unittest.TestCase):
    ...
    def test_model_breakdown(self):
        payload = build_telemetry_payload(model_breakdown={"mini": 2, "max": 1})
        self.assertEqual(payload["model_breakdown"], {"mini": 2, "max": 1})
...
class TestLoadPricingConfig(unittest.TestCase):
    ...
    def test_empty_config_returns_defaults(self):
        ...
        self.assertEqual(result["mini"]["input"], 1.00)
        self.assertEqual(result["mini"]["output"], 5.00)
        self.assertEqual(result["standard"]["input"], 3.00)
        self.assertEqual(result["standard"]["output"], 15.00)
        self.assertEqual(result["max"]["input"], 5.00)
        self.assertEqual(result["max"]["output"], 25.00)
    ...
    def test_full_override(self):
        ...
        result = load_pricing_config(cfg)
        self.assertEqual(result["mini"]["input"], 0.5)
        self.assertEqual(result["mini"]["output"], 2.5)
        self.assertEqual(result["standard"]["input"], 1.5)
        self.assertEqual(result["standard"]["output"], 7.5)
        self.assertEqual(result["max"]["input"], 2.5)
        self.assertEqual(result["max"]["output"], 12.5)
    ...
    def test_partial_override(self):
        ...
        result = load_pricing_config(cfg)
        # Overridden values
        self.assertEqual(result["mini"]["input"], 0.75)
        self.assertEqual(result["max"]["output"], 30.0)
        # Default values
        self.assertEqual(result["mini"]["output"], 5.00)
        self.assertEqual(result["standard"]["input"], 3.00)
        self.assertEqual(result["standard"]["output"], 15.00)
    ...
    def test_string_values_converted(self):
        ...
        self.assertIsInstance(result["mini"]["input"], float)
        self.assertIsInstance(result["standard"]["output"], float)


class TestEstimateAgentCostUsd(unittest.TestCase):
    def test_mini_cheapest(self):
        mini = estimate_agent_cost_usd("mini", input_tokens=50000, output_tokens=10000)
        standard = estimate_agent_cost_usd("standard", input_tokens=50000, output_tokens=10000)
        max = estimate_agent_cost_usd("max", input_tokens=50000, output_tokens=10000)
        self.assertLess(mini, standard)
        self.assertLess(standard, max)

    def test_zero_tokens_zero_cost(self):
        self.assertEqual(estimate_agent_cost_usd("max", input_tokens=0, output_tokens=0), 0.0)

    def test_unknown_model_uses_standard_pricing(self):
        unknown = estimate_agent_cost_usd("gpt-5", input_tokens=1000, output_tokens=1000)
        standard = estimate_agent_cost_usd("standard", input_tokens=1000, output_tokens=1000)
        self.assertEqual(unknown, standard)

    def test_case_insensitive(self):
        lower = estimate_agent_cost_usd("max", input_tokens=10000, output_tokens=5000)
        upper = estimate_agent_cost_usd("max", input_tokens=10000, output_tokens=5000)
        self.assertEqual(lower, upper)


class TestEstimateCampaignSavings(unittest.TestCase):
    def test_mixed_models_cheaper_than_all_max(self):
        agents = [{"model": "mini"}, {"model": "standard"}, {"model": "max"}]
        result = estimate_campaign_savings(agents)
        self.assertGreater(result["savings_pct"], 0)
        self.assertLess(result["tiered_usd"], result["max_usd"])
        self.assertGreater(result["savings_usd"], 0)

    def test_all_max_zero_savings(self):
        agents = [{"model": "max"}, {"model": "max"}]
        result = estimate_campaign_savings(agents)
    ...
    def test_empty_agents(self):
        ...
        self.assertEqual(result["max_usd"], 0)
...
class TestTrackActualTokens(unittest.TestCase):
    ...
    def test_savings_uses_actual_tokens_when_available(self):
        ...
        self.assertLess(result_actual["max_usd"], result_defaults["max_usd"])
...
class TestCmdRunModelField(unittest.TestCase):
    ...
    def test_cmd_run_output_includes_model_field(self):
        cfg = {
            "project": {"name": "Test", "conventions": "AGENTS.md"},
            "commands": {"test": "echo ok"},
            "models": {"low": "mini", "medium": "standard", "high": "max"},
        }
        ...
        # Default complexity is "low" → "mini"
        self.assertEqual(payload["agents"][0]["model"], "mini")
    ...
    def test_cmd_run_model_reflects_task_complexity(self):
        ...
        payload = json.loads(buf.getvalue())
        self.assertEqual(payload["agents"][0]["model"], "max")
...
class TestTieredTokenEstimates(unittest.TestCase):
    ...
    def test_low_tier_cheapest(self):
        ...
        low_agent = [{"model": "standard", "complexity": "low"}]
        high_agent = [{"model": "standard", "complexity": "high"}]
        ...
        self.assertLess(result_low["max_usd"], result_high["max_usd"])

    def test_high_tier_most_expensive(self):
        """Agent with complexity 'high' produces higher cost than 'medium'."""
        medium_agent = [{"model": "standard", "complexity": "medium"}]
        high_agent = [{"model": "standard", "complexity": "high"}]
        ...
        self.assertGreater(result_high["max_usd"], result_medium["max_usd"])
    ...
    def test_tiered_differs_from_flat(self):
        ...
        agents = [{"model": "standard", "complexity": "low"}, {"model": "standard", "complexity": "high"}]
        ...
        self.assertNotEqual(result_tiered["max_usd"], result_flat["max_usd"])

    def test_missing_complexity_falls_back(self):
        """Agent with no complexity key falls back to 'medium' tier budget."""
        no_complexity = [{"model": "standard"}]
        medium_complexity = [{"model": "standard", "complexity": "medium"}]
        ...
        self.assertEqual(result_no_key["max_usd"], result_medium["max_usd"])
