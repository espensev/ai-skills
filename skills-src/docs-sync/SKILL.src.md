...
# Docs Sync {{dash}} Documentation Surface Consistency
...
**Config:** `.{{provider-lc}}/skills/project.toml` {{dash}} doc surfaces in `[docs-sync]`
...
|---------|-------|---------|
| `check` | `{{cmd}}docs-sync` or `{{cmd}}docs-sync check` | Full consistency scan |
| `surface` | `{{cmd}}docs-sync surface README.md` | Check a single doc against code |
| `fix` | `{{cmd}}docs-sync fix` | Auto-fix trivial inconsistencies |
...
[docs-sync.tier1]
# High-traffic docs {{dash}} checked thoroughly
files = ["{{ctx}}", "README.md", "STATUS.md", "CONTRIBUTING.md"]

[docs-sync.tier2]
# Planning and architecture docs {{dash}} checked for key claims
...
1. Auto-discover documentation files:
   - Tier 1: `README.md`, `{{ctx}}`, `CONTRIBUTING.md`, `STATUS.md`
...
### Tier 1 {{dash}} High-traffic (checked thoroughly)

These docs are frequently read by humans and AI agents. Contradictions here
cause the most damage.

Common surfaces:
- `README.md` {{dash}} setup instructions, feature list, build commands
- `{{ctx}}` {{dash}} AI agent instructions, project conventions
- `STATUS.md` {{dash}} current state, metrics, build status
- `CONTRIBUTING.md` {{dash}} build/test commands, dev setup

### Tier 2 {{dash}} Planning docs (checked for key claims)

These inform architecture decisions. Stale claims lead to bad plans.

Common surfaces:
- `docs/architecture/*.md` {{dash}} system design, module descriptions
- `docs/planning/*.md` {{dash}} roadmaps, contracts, tech specs
- `docs/api/*.md` {{dash}} endpoint documentation
...
{{#claude}}
Check: file/directory exists via Glob
{{/claude}}
{{#codex}}
...
Check: command/script exists (not executed {{dash}} just verify it's defined)
```
...
## Command: `check` {{dash}} Full Consistency Scan
...
```
Docs Sync {{dash}} Check

  Conflict Markers:
    PASS {{dash}} no conflict markers found

  Path Existence:
    FAIL {{dash}} README.md:45 references "src/old-module.py" (does not exist)
    PASS {{dash}} 23/24 paths verified

  Version Sync:
    PASS {{dash}} version 2.1.0 consistent across 4 surfaces

  Build Commands:
    PASS {{dash}} all documented commands exist

  Cross-Doc Consistency:
    FAIL {{dash}} {{ctx}} says port 8080, CONTRIBUTING.md says port 3000

  Staleness:
    WARN {{dash}} STATUS.md last updated 12 days ago
...
## Command: `surface` {{dash}} Single Document Check

Runs all checks scoped to one file. Faster for targeted verification.

Usage: `{{cmd}}docs-sync surface CONTRIBUTING.md`
...
## Command: `fix` {{dash}} Auto-Fix Trivial Issues

{{#claude}}
Auto-fixes (using Edit tool):
{{/claude}}
{{#codex}}
Auto-fixes:
{{/codex}}
...
|-------|--------|---------|
| `{{cmd}}schema-validator` | Code {{bidir}} Code | Schema authority vs consumer layers |
| `{{cmd}}truthpack-drift` | Truth {{bidir}} Code | Declared truth stores vs actual codebase |
| `{{cmd}}docs-sync` | Docs {{bidir}} Code + Docs {{bidir}} Docs | Documentation vs code and cross-doc consistency |
...
|-------|---------------------|
| `{{cmd}}manager verify` | Include docs-sync check in campaign exit criteria |
| `{{cmd}}schema-validator` | Parallel validator {{dash}} schema checks code-to-code; docs-sync checks code-to-docs |
| `{{cmd}}truthpack-drift` | Parallel validator {{dash}} truthpack checks truth-to-code; docs-sync checks docs |
| `{{cmd}}ship` | Quick check before commit catches stale doc references |
| `{{cmd}}observer` | Record `drift` observations for documentation divergence |
| `{{cmd}}qa` | Docs-sync complements code quality with documentation quality |
...
- Never delete documentation {{dash}} only update or flag
