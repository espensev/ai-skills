...
Works with campaign agents (from `{{cmd}}manager` orchestration) and, when
hooks instrumentation is available, ad-hoc subagent spawns. Complements
`{{cmd}}campaign-health` (state and stuck detection) with performance-focused
metrics.

**All commands run to completion autonomously.**

**Config:** `.{{provider-lc}}/skills/project.toml` — uses `[paths]` for campaign state
...
|---------|-------|---------|
| `summary` | `{{cmd}}agent-report` or `{{cmd}}agent-report summary` | Agent overview across recent activity |
| `detail` | `{{cmd}}agent-report detail <agent-id>` | Deep metrics for a specific agent |
| `efficiency` | `{{cmd}}agent-report efficiency` | Model selection cost analysis |
| `trends` | `{{cmd}}agent-report trends [N]` | Cross-campaign performance trends (default 5) |
...
1. **Hooks JSONL log** (subagent events with timestamps and tool attribution
   ...
   ```bash
   grep -c 'SubagentStart\|SubagentStop' .{{provider-lc}}/hooks/logs/hooks-log.jsonl 2>/dev/null
   ```
...
{{/codex}}
  ...
  Campaign Agents (active campaign):
    ...
    C-migration      {{model-mid-lc}}   done       4    medium
    D-integration    {{model-high-lc}}     done       2    high
    E-tests          {{model-low-lc}}    running    3    low
    F-docs           {{model-low-lc}}    pending    2    low
```
...
2. **Calculate cost scenarios** using `estimate_campaign_savings()` from
   ...
   - All-{{Model-high}} cost (what if every agent used {{Model-high}})
   - All-{{Model-low}} cost (what if every agent used {{Model-low}})
   - Savings from tiered selection vs all-{{Model-high}}

3. **Evaluate model-task fit**:
   - Did high-complexity tasks assigned to {{Model-high}} succeed without retries?
   - Did low-complexity tasks assigned to {{Model-low}} succeed?
...
Agent Report — Model Efficiency
  ...
  Per-Model Tool Efficiency (when hooks data is available):
    ...
    ─────────────────────────────────────────────────
    {{model-low-lc}}      8.3             4m 20s       $0.025
    {{model-mid-lc}}    16.0            12m 30s       $0.026
    {{model-high-lc}}      22.0            18m 45s       $0.017
...
```
  Recommendation:
    - Agent A-contracts used {{model-high-lc}} for a low-complexity task — consider {{model-low-lc}}
    - Agent E-tests failed twice with {{model-low-lc}} — consider upgrading to {{model-mid-lc}}
```
...
3. **Compute trends over time**:
   ...
   - Is model selection becoming more aggressive (shifting toward {{model-low-lc}})?
...
Agent Report — Trends (last 5 campaigns)
  ...
  Trends (5-campaign window):
    ...
    {{Model-low}} usage:     increasing (25% -> 50%)

  Recommendations:
    Success rate is strong. Consider:
    - Moving more medium-complexity tasks to {{model-low-lc}} for further savings
...
|-------|------------------------|
| `{{cmd}}session-stats` | Provides tool counts that feed per-agent efficiency analysis |
| `{{cmd}}token-audit` | Provides cost data for per-agent cost attribution |
| `{{cmd}}campaign-health` | Agent-report adds performance dimension to health checks |
| `{{cmd}}planner` | Trend data informs model selection for future campaign planning |
| `{{cmd}}manager verify` | Include agent efficiency in campaign verification checks |
| `{{cmd}}observer` | Record efficiency findings as project-level observations |
...
- When comparing models, normalize by complexity tier (don't compare {{model-low-lc}} on easy tasks against {{model-high-lc}} on hard tasks)
