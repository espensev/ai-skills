...
# Approximate per-million-token pricing (USD) — kept as simple constants.
# Updated from public Anthropic pricing as of 2025-05.  Only used for
# *estimates*; real billing comes from the API provider.
...
def load_pricing_config(cfg: dict) -> dict[str, dict[str, float]]:
    ...
    Looks for keys like 'haiku_input', 'haiku_output', etc. in cfg.get("pricing", {}).
...
def estimate_campaign_savings(
    ...
    *,
    ...
    """Compare estimated cost of the tiered model selection vs. all-Opus.

    *agents* should be a list of dicts with at least a ``model`` key.
    Returns a dict with ``tiered_usd``, ``opus_usd``, ``savings_usd``, and
    ``savings_pct``.
    ...
    return {
        ...
        "savings_pct": round(savings / opus * 100, 1) if opus else 0.0,
    }
