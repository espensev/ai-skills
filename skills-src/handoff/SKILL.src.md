...
**Output:** a brief status {{dash}} where things stand, which claims are stale,
...
- When the user asks to park other findings, route a concise durable note into
  the requested documentation work {{dash}} evidence, ownership boundaries,
...
class {{dash}} say what could not be checked and why, rather than passing it
through as fact.
...
| "file `<path>` has Y" | read the file, or compare its recorded hash {{dash}} it may have moved |
...
| "next step is Z" | the latest tracker and later commits {{dash}} is Z already done? |
...
1. **Where things actually stand** {{dash}} the verified picture, not the
   note's picture.
...
{{/codex}}
| An explicitly requested multi-agent campaign | `{{cmd}}manager` {{dash}} never for ordinary resumed work |
...
naming the next gate is not an outcome {{dash}} stop only when an unanswered
...
- Steps 1{{dash}}3 are read-only. Do not merge, push, delete, or reset during
...
- A bounded Remember handoff and provider-native memory are separate. Native
  ...
  route; `{{cmd}}memory-management` governs that route.
