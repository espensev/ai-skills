...
**Config:** `.{{provider-lc}}/skills/project.toml` — `[token-audit]` and `[pricing]` sections (optional)
...
|---------|-------|---------|
| `status` | `{{cmd}}token-audit` or `{{cmd}}token-audit status` | Current token position and burn rate |
| `breakdown` | `{{cmd}}token-audit breakdown` | Per-tool and per-turn token cost attribution |
| `budget` | `{{cmd}}token-audit budget [set <N>|check|reset]` | Session or daily budget management |
| `forecast` | `{{cmd}}token-audit forecast` | Rate-limit proximity and depletion forecast |
| `history` | `{{cmd}}token-audit history [N]` | Usage trends across last N sessions (default 7) |
...
2. **Message role weighting**:
   ...
   - System messages and {{ctx}}: count as input tokens (amortize across turns)
...
4. **Compute overhead**: Estimate system prompt, conversation history, and
   {{ctx}} content that is sent with every request but not attributable to
   a single tool.
...
#### `{{cmd}}token-audit budget set <amount>`
...
{{/codex}}
    ...
    - Prefer `{{model-low-lc}}`-tier agents for research tasks
```
...
|-------|----------------------|
| `{{cmd}}session-stats` | Token-audit adds cost dimension to session tool analytics |
| `{{cmd}}campaign-health` | Token costs feed into campaign efficiency metrics |
| `{{cmd}}planner` | Budget awareness informs agent model selection decisions |
| `{{cmd}}manager` | Cost tracking per campaign validates tiered-model savings |
| `{{cmd}}agent-report` | Per-agent cost data feeds agent efficiency analysis |
| `{{cmd}}observer` | Budget alerts can be recorded as observations |
