...
**Default command:** `{{cmd}}diagnosing-bugs <symptom>`
...
|---------|-------|---------|
| `loop` | `{{cmd}}diagnosing-bugs loop <symptom>` | Build or tighten the red-capable feedback loop. |
| `perf` | `{{cmd}}diagnosing-bugs perf <symptom>` | Establish timing/profiling evidence before fixing a performance regression. |
| `fix` | `{{cmd}}diagnosing-bugs fix <symptom>` | Reproduce, minimize, fix, and verify the bug. |
| `postmortem` | `{{cmd}}diagnosing-bugs postmortem` | Capture cause, regression coverage, and prevention notes after the fix. |
...
Read local context first: `CONTEXT.md`, `{{ctx}}`, nearby ADRs, failing test
