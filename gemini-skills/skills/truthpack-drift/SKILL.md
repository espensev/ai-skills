...
4. **Read project.toml first:** all project-specific config lives in `.gemini/skills/project.toml` under `[truthpack-drift]`.
...
## Setup: Read Config
Before any command: read `.gemini/skills/project.toml` and extract the `[truthpack-drift]` section.
