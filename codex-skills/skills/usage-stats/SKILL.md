...
2. **Message role weighting**: user messages, tool results returned to the
   model, and system/AGENTS.md content count as input tokens (amortize
...
5. **Compute overhead**: system prompt, conversation history, and AGENTS.md
...
3. **Pace adjustments**: if burning faster than sustainable, recommend
   ...
   fewer turns, and `mini`-tier agents for research tasks; if
...
Usage Stats — Agent Detail: B-core-refactor
  ...
  Type: Campaign agent (Plan: plan-012)   Model: standard
...
2. **Calculate cost scenarios** using `estimate_campaign_savings()` from
   ...
   all-Max cost, all-Mini cost, savings from tiered
   selection vs all-Max.

3. **Evaluate model-task fit**: did high-complexity tasks on Max
   succeed without retries? Did low-complexity tasks on Mini succeed?
...
Usage Stats — Model Efficiency
  ...
  Cost Comparison:
    Tiered (actual) $1.85 | All-Max $3.40 | All-Mini $0.68
...
max for a low-complexity task — consider mini" or
"Agent E-tests failed twice with mini — upgrade to standard".
...
3. **Compute trends over time**: campaign size, cost per agent, success rate,
   model-selection aggressiveness (share of mini).
...
Usage Stats — Trends (last 5 campaigns)
  ...
  Trends: success improving (88% -> 100%); cost/agent decreasing
          ($0.40 -> $0.31, -22%); Mini usage increasing (25% -> 50%)

  Recommendations:
    - Move more medium-complexity tasks to mini for further savings
...
# hooks_log = ".codex/hooks/logs/hooks-log.jsonl"
# history_sessions = 10
...
- When comparing models, normalize by complexity tier (don't compare
  mini on easy tasks against max on hard tasks)
