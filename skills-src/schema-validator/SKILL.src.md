...
# Schema Validator {{dash}} Cross-Layer Contract Checker

Validates that the authoritative schema definition is correctly consumed by
all downstream layers {{dash}} data access code, API contracts, and contract tests.
Catches cross-layer drift before it becomes a runtime failure.

**All commands run to completion autonomously.**

**Config:** `.{{provider-lc}}/skills/project.toml` {{dash}} schema surfaces configured in `[schema-validator]`
...
|---------|-------|---------|
| `check` | `{{cmd}}schema-validator` or `{{cmd}}schema-validator check` | Full cross-layer validation |
| `drift` | `{{cmd}}schema-validator drift` | Quick drift detection {{dash}} tables and columns only |
| `report` | `{{cmd}}schema-validator report` | Generate cross-reference report to docs/ |
...
```toml
[schema-validator]
# The authoritative schema source {{dash}} the single point of truth.
...
# Consumer layers {{dash}} code that reads or writes against the schema.
...
1. Auto-detect schema type from project files:
   - `*.sql` files {{arrow}} SQL
   - `prisma/schema.prisma` {{arrow}} Prisma
   - `drizzle.config.ts` {{arrow}} Drizzle
   - `*.proto` {{arrow}} Protobuf
   - `openapi.yaml/json` {{arrow}} OpenAPI
...
## Command: `check` {{dash}} Full Validation
...
4. **Cross-reference all three layers:**
   ...
   | Check | Rule |
   |-------|------|
   | Consumer {{arrow}} Authority | Every column a consumer references must exist in schema |
   | Authority {{arrow}} Consumer | Every table should have at least one consumer (warn if orphaned) |
   | Authority {{arrow}} Tests | Every table should have contract test coverage |
   | Consumer {{arrow}} Tests | Every consumer query pattern should be tested |
...
Schema Validator {{dash}} Check
  ...
  (unknown)      {{dash}}           1 ref       {{dash}}          FAIL (consumer refs missing entity)

  Detail:
    FAIL: src/services/DataService.ts:42 references column "events.metadata"
          which does not exist in schema.sql

  Schema version: 12 (authority) {{dash}} all docs match
...
## Command: `drift` {{dash}} Quick Check

Tables and columns only {{dash}} no query parsing, no test coverage analysis.
...
## Command: `report` {{dash}} Documentation
...
`/schema-validator check` + `/truthpack-drift check` + `/docs-sync check`
{{/claude}}
{{#codex}}
**Run all three together** for comprehensive drift detection.
{{/codex}}
...
`drift` observations to `{{cmd}}observer`.

## Integration

| Skill | How Schema Validator Helps |
|-------|--------------------------|
| `{{cmd}}build-gate` | Build-gate checks compilation; schema-validator checks data contracts |
| `{{cmd}}smart-test` | When schema files change, add contract tests to the test set |
| `{{cmd}}planner` | Campaigns touching schema should include schema-validator as exit criterion |
| `{{cmd}}truthpack-drift` | Parallel validator {{dash}} schema checks code-to-code; truthpack checks truth-to-code |
| `{{cmd}}docs-sync` | Parallel validator {{dash}} schema checks code-to-code; docs-sync checks code-to-docs |
| `{{cmd}}observer` | Record `drift` observations for schema divergence |
| `{{cmd}}qa` | Schema validation complements test-based quality checks |
...
- The authority is always right {{dash}} consumers and tests must match it
- Report all entities even when most pass (visibility)
- Exit 0 on PASS/WARN, exit 1 on FAIL
- Never modify schema or consumer code {{dash}} only report
