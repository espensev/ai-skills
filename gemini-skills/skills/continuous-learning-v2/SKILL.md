...
Treat these as strong learning inputs:

- explicit user correction or rejection
- repeated QA or verification failure with the same root cause
- repeated eval misses for the same skill behavior
- stable workflow preference observed across multiple sessions

Do not promote these on their own:

- single flaky test failures
- ambiguous logs without a confirmed root cause
- provider-specific path leakage or wording copied from another package
- one-off stylistic preferences unless the user explicitly asks to codify them

Learning quality depends more on promotion discipline than on raw observation volume.
...
```json
{
  "hooks": {
    "PreToolUse": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "${GEMINI_PLUGIN_ROOT}/skills/continuous-learning-v2/hooks/observe.sh"
      }]
    }],
    "PostToolUse": [{
      "matcher": "*",
...
**If installed manually** to `~/.gemini/skills`:
