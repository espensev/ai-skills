...
This document explains how `.codex/skills/project.toml` works, including the
...
file at `.codex/skills/project.toml`. This file tells the runtime where to
...
2. **Render** the template (`project.toml.template`) into the final config
   (`.codex/skills/project.toml`) by substituting placeholders with
   detected values.
...
| `.codex/skills/project.toml` | Rendered, project-specific config. Created by `init`. |
...
2. If `.codex/skills/project.toml` does not exist (or `--force` is passed):
...
| `{{CONVENTIONS_PATH}}` | JSON-quoted path to conventions file (default: `"AGENTS.md"`) | `"AGENTS.md"` |
...
```
Next: edit .codex/skills/project.toml and fill in [commands].test
```
...
| Key | Default | Description |
|-----|---------|-------------|
| `low` | `"mini"` | Model for low-complexity agents. Cheapest, fastest. |
| `medium` | `"standard"` | Model for medium-complexity agents. Good balance. |
| `high` | `"max"` | Model for high-complexity agents. Most capable. |
