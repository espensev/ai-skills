...
## Execution Rules
1. **Light guardrails:** Follow the global guardrails in AGENTS.md. This is a read-only ops/analytics skill — do not gate it on the planning workflow.
...
# hooks_log = ".agent/hooks/logs/hooks-log.jsonl"
# history_sessions = 10
...
2. **Hooks JSONL log** (tier 2 — richest local source for tool events): `ls -la .agent/hooks/logs/hooks-log.jsonl 2>/dev/null`. If present, contains timestamped tool events with session_id, tool_name, tool_input, agent_id.
