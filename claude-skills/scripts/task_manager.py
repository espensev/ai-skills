...
Reads project config from .claude/skills/project.toml (optional).
...
# ---------------------------------------------------------------------------
# Project config — loaded from .claude/skills/project.toml
# ---------------------------------------------------------------------------
...
CONVENTIONS_FILE = _CFG.get("project", {}).get("conventions", "CLAUDE.md")
...
def _preflight_safe_fix() -> list[str]:
    ...
    # Copy planning-contract.md to .claude/skills/ if source exists and dest does not
    ...
    # Create CLAUDE.md stub if conventions file is the default and doesn't exist
    conventions_value = str(CONVENTIONS_FILE or "").strip()
    if conventions_value == "CLAUDE.md":
...
def _plan_preflight_payload() -> dict:
    ...
    if not config_path.exists():
        errors.append("Missing .claude/skills/project.toml. Run init --force first.")
    if not contract_path.exists():
        errors.append("Missing .claude/skills/planning-contract.md in the installed runtime.")
...
def _finalize_plan_updates(plan: dict, args) -> tuple[dict, list[str], list[str], list[str]]:
    ...
    agents = plan.get("agents", [])
    if agents:
        _complexity_to_model = {"low": "haiku", "medium": "sonnet", "high": "opus"}
        agent_dicts = [
            {
                "model": _complexity_to_model.get(str(a.get("complexity", "medium") or "medium").lower(), "sonnet"),
...
def main():
    ...
    pc.add_argument("--planner-kind", default="planner", choices=["planner", "refactor-planner", "manager-go"])
