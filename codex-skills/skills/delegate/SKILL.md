...
sub-task to a LOCAL Ollama model, or keep it with the controller - then route it when
...
|---------|-------|---------|
| `guidance` | `$delegate guidance` | Show the static safe-task split + client compatibility. Works with NO fleet and even when the MCP server is absent. |
| `check` | `$delegate check` | Report whether local delegation is usable right now (`delegation_usable` / `degraded`). |
| `<task>` | `$delegate <task description>` | Classify the task, refuse if it is a non-delegatable category, else route it and surface the verify hint and savings. |
| `batch` | `$delegate batch` | Route 3+ similar independent items in parallel. |
...
the current default mapping; do not hardcode it as a routing decision - pass the
...
- **`transform`** - the one task type hard-blocked in `dispatch-rules.json`
...
When refusing, say plainly: *"Keep this with the controller - <reason>."* Do not call
...
1. **MCP server not registered.** If the `ollama_*` tools are absent from this host,
   ...
   answer `$delegate guidance` and classify tasks from this document alone.
...
3. **Controller owns the output.** Local delegation is for narrow, reviewable sub-tasks
   ...
   destructive work - see the refuse list above.
4. **Escalation is honored.** If a delegation returns `status: "escalate"`, treat it as
   "the local model declined" - the controller takes the task back.

---

## Command: `guidance` - Static Safe-Task Split

Goal: show when to delegate, when not to, and which path is available - without touching
the telemetry API.
...
2. If the MCP server is **not** registered, present the static split from the
   **Task Classification** section above and note that active routing is unavailable
   here - the controller should do bounded tasks itself.
...
Delegate - Guidance
  ...
  Rule        : controller fetches -> local transforms -> controller verifies
```

---

## Command: `check` - Is Delegation Usable Now?

Goal: a fast "should I reach for local delegation at all?" answer.

### Steps

1. If the MCP server is not registered -> report `routing: static-only`, advise
...
## Command: `<task description>` - Classify, then Route or Refuse
...
1. **Classify** the described task against the **Task Classification** table.
   - If it is a non-delegatable category -> **REFUSE**: tell the controller to handle it
...
3. **Availability check.**
   - MCP not registered -> fall back to static guidance, stay with controller.
   - Else call `ollama_readiness`; if not `delegation_usable` -> stay with controller.
...
5. **Handle the result:**
   - `status: "ok"` -> surface the response, the returned `verify_hint`, and the savings
     from `metrics.sonnet_equivalent_cost_usd` / `session_totals.sonnet_equivalent_saved_usd`.
   - `status: "escalate"` -> the local model declined; controller takes it back.
   - `status: "error"` (including blocked task_type) -> report the error and keep the
...
Delegate - Route
  ...
  Status      : ok
  Verify      : <verify_hint - what the controller must confirm>
...
## Command: `batch` - 3+ Similar Independent Items
...
3. Call `ollama_batch_delegate` with a `tasks` array; each task carries `id`,
   ...
   set - the server auto-chunks to the fleet's parallel width (`batch_max`); do not
   pre-split.
...
Delegate - Batch
  ...
  Verify      : <per-task verify hints - controller confirms each>
```
...
- **MCP-optional:** never error because the MCP server is absent - fall back to static
...
- Default command is `guidance`, not a route - classify before acting.
