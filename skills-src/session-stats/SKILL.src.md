...
**Config:** `.{{provider-lc}}/skills/project.toml` — `[session-stats]` section (optional)
...
|---------|-------|---------|
| `summary` | `{{cmd}}session-stats` or `{{cmd}}session-stats summary` | Quick session overview |
| `tools` | `{{cmd}}session-stats tools` | Tool usage breakdown with counts and patterns |
| `agents` | `{{cmd}}session-stats agents` | Subagent spawning, types, and outcomes |
| `timeline` | `{{cmd}}session-stats timeline` | Activity timeline with phases |
| `compare` | `{{cmd}}session-stats compare [N]` | Compare last N sessions (default 5) |
| `export` | `{{cmd}}session-stats export` | Export session metrics as JSON |
...
2. **Hooks JSONL log** (tier 2 — richest local source for tool events, if
   ...
   ```bash
   ls -la .{{provider-lc}}/hooks/logs/hooks-log.jsonl 2>/dev/null
   ```
...
2. **Count tool calls** by type — from telemetry per-session data when tier 1
   ...
   c = Counter()
   for line in open('.{{provider-lc}}/hooks/logs/hooks-log.jsonl'):
       try:
           e = json.loads(line)
...
3. **Count agent spawns** (SubagentStart/SubagentStop events, if hooks
   ...
   ```bash
   grep -c '"SubagentStart"' .{{provider-lc}}/hooks/logs/hooks-log.jsonl 2>/dev/null
   ```
...
Session Stats — Agent Activity
  ...
  Campaign Agents (if active):
    Agent A: 4 files, model={{model-mid-lc}}, status=done
    Agent B: 3 files, model={{model-low-lc}}, status=running
```
...
|-------|------------------------|
| `{{cmd}}observer` | Feed tool patterns as observations for project intelligence |
| `{{cmd}}campaign-health` | Supplement campaign metrics with session-level tool detail |
| `{{cmd}}token-audit` | Session-stats provides tool counts; token-audit adds cost analysis |
| `{{cmd}}agent-report` | Agent counts from session-stats feed agent performance analytics |
