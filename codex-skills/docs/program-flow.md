...
5. Emit launch JSON:
   {
     ...
     "agents": [{
       "id": "a",
       "model": "standard",        ← from complexity → model mapping
       "isolation": "worktree",
       "background": true,
...
The launch JSON is consumed by the Codex CLI to spawn agents in
...
```python
model_breakdown = {"mini": 2, "standard": 3, "max": 1}
```
...
```python
_MODEL_PRICING = {
    "mini":  {"input": 1.00,  "output": 5.00},   # per 1M tokens
    "standard": {"input": 3.00,  "output": 15.00},
    "max":   {"input": 5.00,  "output": 25.00},
}
```

`estimate_campaign_savings()` compares tiered model costs vs all-max baseline
...
live-tracker.md ─ sync ───────┘                         │
                                                        │
tasks.json ──> run ──> launch JSON ──> Codex CLI  │
                                           │            │
                                    [worktree agent]    │
                                           │            │
                                     result JSON ───────┘
