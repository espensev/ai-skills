...
- **Configuration loading** — reads `.claude/skills/project.toml`, derives runtime paths
...
| File | Purpose |
|------|---------|
| `.claude/skills/project.toml` | Project-local config: `[project]`, `[paths]`, `[commands]`, `[analysis]`, `[models]` sections |
| `project.toml.template` | Template rendered by `init --force` for new consumer repos |
| `CLAUDE.md` | Project conventions file — every agent reads this first |
