...
|---------|-------|---------|
| `status` | `{{cmd}}manager` or `{{cmd}}manager status` | Show task state + dependency graph |
| `analyze` | `{{cmd}}manager analyze` | Scan project structure, files, imports |
| `go` | `{{cmd}}manager go <description>` | Skill-level workflow: plan → fill specs → launch → auto-advance → merge → verify |
| `plan` | `{{cmd}}manager plan <description>` | Plan + register + fill specs (stops before launch) |
| `run` | `{{cmd}}manager run <agents\|ready>` | Launch agents + auto-advance through all groups |
| `merge` | `{{cmd}}manager merge` | Skill-level workflow: merge completed agent worktrees into main working tree |
| `verify` | `{{cmd}}manager verify` | Skill-level workflow: post-merge validation + readiness assessment |
| `new` | `{{cmd}}manager new <name> [scope]` | Quick-add a single agent |
| `review` | `{{cmd}}manager review <agent>` | Review a completed agent's work: read spec, check diff, mark complete |
| `next` | `{{cmd}}manager next` | Auto-advance: launch whatever is ready |
...
| Scenario | Use | Why |
|----------|-----|-----|
| Feature work with clear scope | `{{cmd}}manager go` | End-to-end autonomous; fastest path |
| First campaign in unfamiliar codebase | `{{cmd}}manager plan` then review, then `{{cmd}}manager run ready` | Lets you inspect the plan and specs before committing |
| Refactor with high coordination cost | `{{cmd}}manager plan` then review | Refactors benefit from human sign-off on decomposition |
| Quick fix (1-2 agents) | `{{cmd}}manager go` | Overhead of review outweighs risk |
| Follow-up campaign after a failure | `{{cmd}}manager plan` then review | Understand what went wrong before re-executing |
...
1. Read `.{{provider-lc}}/skills/project.toml` — project config (paths, commands, modules)
2. Read `.{{provider-lc}}/skills/planning-contract.md` — the shared planning contract
...
Read `.{{provider-lc}}/skills/planning-contract.md` — the shared planning contract that
...
the user can launch with `{{cmd}}manager run ready`.
...
Read `.{{provider-lc}}/skills/project.toml` for the project's build/test/compile commands.
...
3. In the final summary, recommend: `{{cmd}}discover {targeted question}` followed by
...
**When to update:**
- After `{{cmd}}manager review` marks an agent complete (step 5 of review)
- After `{{cmd}}manager verify` passes (append a campaign summary row)
- After `{{cmd}}manager go` completes the full lifecycle
...
**Ship behavior:** `{{cmd}}ship` classifies the tracker file as a "warn" file —
