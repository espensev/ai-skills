...
## Execution Rules
1. **Follow the global guardrails in GEMINI.md.** (This is an ops/analytics
...
**Config:** `.gemini/skills/project.toml` — `[token-audit]` and `[pricing]`
sections (optional).
...
- **Tier 2 — Hooks JSONL log (semi-measured):** tool call metadata with
  timestamps. `ls .gemini/hooks/logs/hooks-log.jsonl 2>/dev/null`
...
- Rate-limit cache (if a Gemini statusline or similar surface is active).
...
2. **Message role weighting:** user messages + tool results → input tokens;
   ...
   GEMINI.md → input tokens (amortize across turns).
...
4. **Compute overhead:** system prompt, conversation history, and GEMINI.md sent
...
Token Audit — Breakdown
  ...
  * Overhead: system prompts, GEMINI.md, conversation history
