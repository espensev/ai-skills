...
## Core Mandate
Given a set of changed files, determine and run the minimal test subset instead of the full test suite. Read module definitions from project.toml and build source-to-test mappings dynamically. Follow the global guardrails in AGENTS.md.

**All commands run to completion autonomously.**

**Config:** `.agents/skills/project.toml` — modules, test commands, conflict zones
...
## Setup: Load Config
Before any command:
1. Read `.agents/skills/project.toml`
...
1. **Explicit mappings (highest priority)** — if `[smart-test.mappings]` is configured, use those first. Exact source → test file relationships defined by the user.
2. **Module-based mapping** — use `[modules]` from project.toml. For each changed source file, find which module it belongs to, then search test files for imports of that module name (`import <module>`, `from <module>`, `require.*<module>`, etc.).
3. **Convention-based mapping** — if no module match, try naming conventions:
...
4. **Import-based discovery (fallback)** — search all test files for references to the changed file's exports: function names, class names, type names, file path references.

### Cross-cutting files
Some files trigger the full suite when changed:
- Auto-detected: `conftest.py`, `jest.config.*`, `setup.cfg`, `pyproject.toml`, `package.json`, `tsconfig.json`, `Cargo.toml`, `CMakeLists.txt`
...
2. Group resolved test files by test harness:
   ...
   - Other → `[commands].test`
3. Run each harness with its scoped test file list: `<test-command> <test-file-1> <test-file-2> ...`
...
2. Search test files for imports of the changed module.
...
Smart test is a **speed optimization**, not a replacement for qa. Use it for fast iteration; always run `/qa` or `/build-gate all` before merge/ship.
