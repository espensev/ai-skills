...
**Config:** `.{{provider-lc}}/skills/project.toml` — `[usage-stats]` and `[pricing]` sections (optional)
...
| `history` | `{{cmd}}usage-stats history [N]` | Usage trends across last N sessions (default 7) |

**Session analytics** (formerly session-stats):

| Command | Usage | Purpose |
|---------|-------|---------|
| `summary` | `{{cmd}}usage-stats` or `{{cmd}}usage-stats summary` | Quick session overview |
| `closeout` | `{{cmd}}usage-stats closeout` | Compact end-of-execution outcome, time, tokens, evidence, and risk |
| `tools` | `{{cmd}}usage-stats tools` | Tool usage breakdown with counts and patterns |
| `timeline` | `{{cmd}}usage-stats timeline` | Activity timeline with phases |
| `compare` | `{{cmd}}usage-stats compare [N]` | Compare last N sessions (default 5) |
| `export` | `{{cmd}}usage-stats export` | Export session metrics as JSON |

**Agent performance** (formerly agent-report):

| Command | Usage | Purpose |
|---------|-------|---------|
| `agents` | `{{cmd}}usage-stats agents [agent-id]` | Agent overview, or deep dive on one agent |
| `efficiency` | `{{cmd}}usage-stats efficiency` | Model selection cost analysis |
| `trends` | `{{cmd}}usage-stats trends [N]` | Cross-campaign performance trends (default 5) |
...
{{#codex}}
1b. **Native Codex session counters** (Tier 1 — measured). Locate the current
   session JSONL under the active Codex home only when session ID, current
   project, or the current prompt identifies it unambiguously. Parse the last
   ...
   `total_tokens`. Input already includes cached input, and output already
   includes reasoning output; show cached/reasoning as subsets and never add
...
2. **Hooks JSONL log** (Tier 2 — richest local source for tool events, if
   ...
   ```bash
   ls .{{provider-lc}}/hooks/logs/hooks-log.jsonl 2>/dev/null
   ```
...
{{/claude}}
{{#codex}}
   Location varies by Codex installation; check the active Codex home and
   common defaults. Native `token_count` events provide measured totals; use
   other transcript records for tool, phase, and per-turn attribution. Fall
   back to hooks-log tool sizes if no transcript is accessible.
{{/codex}}
...
{{/claude}}
{{#codex}}
When neither native Codex `token_count` events nor telemetry are available,
fall back to this estimation pipeline.
{{/codex}}
...
2. **Message role weighting**: user messages, tool results returned to the
   model, and system/{{ctx}} content count as input tokens (amortize
...
5. **Compute overhead**: system prompt, conversation history, and {{ctx}}
...
Usage Stats — Breakdown
  ...
  * Overhead: system prompts, {{ctx}}, conversation history
...
#### `{{cmd}}usage-stats budget set <amount>`
...
{{/claude}}
{{#codex}}
   reduced effort level, Read with
{{/codex}}
   offset/limit instead of full-file reads, batching related questions into
   fewer turns, and `{{model-low-lc}}`-tier agents for research tasks; if
...
{{/claude}}
{{#codex}}
2. **Elapsed** — measure wall time from the turn's `task_started` event to the
   latest event/report clock. If only session-level timestamps exist, label the
   duration as session time rather than turn time.
{{/codex}}
...
{{/claude}}
{{#codex}}
   extended (MCP tools `mcp__*`, WebFetch, WebSearch), and any
{{/codex}}
...
`{{cmd}}manager` orchestration) with performance-focused metrics.

### `{{cmd}}usage-stats agents` — overview
...
{{/claude}}
{{#codex}}
    By type: research 2 (avg 0:58, 100%), general 1 (avg 3:22, 100%)
{{/codex}}
  ...
  Campaign Agents (active campaign): 6 agents across 1 plan
    ...
    ──────────────────────────────────────────────────────
    A-contracts      {{model-low-lc}}    done       3    low
    B-core-refactor  {{model-mid-lc}}   done       5    medium
    D-integration    {{model-high-lc}}     done       2    high
```

### `{{cmd}}usage-stats agents <agent-id>` — deep dive
...
Usage Stats — Agent Detail: B-core-refactor
  ...
  Type: Campaign agent (Plan: plan-012)   Model: {{model-mid-lc}}
  ...
  Estimated Cost: $0.42 (input ~35K, output ~8K at {{model-mid-lc}} rates)
...
2. **Calculate cost scenarios** using `estimate_campaign_savings()` from
   ...
   all-{{Model-high}} cost, all-{{Model-low}} cost, savings from tiered
   selection vs all-{{Model-high}}.

3. **Evaluate model-task fit**: did high-complexity tasks on {{Model-high}}
   succeed without retries? Did low-complexity tasks on {{Model-low}} succeed?
...
Usage Stats — Model Efficiency
  ...
  Cost Comparison:
    Tiered (actual) $1.85 | All-{{Model-high}} $3.40 | All-{{Model-low}} $0.68
    Savings vs {{Model-high}}: $1.55 (45.6%)

  Model Distribution:
    {{model-low-lc}}:   3 agents (low complexity)   — $0.21 avg, 100% success
    {{model-mid-lc}}:  2 agents (medium complexity) — $0.42 avg, 100% success
    {{model-high-lc}}:    1 agent  (high complexity)   — $0.38 avg, 100% success

  Recommendation: Current tiered selection is efficient.
```

If inefficiencies are detected, name them — e.g. "Agent A-contracts used
{{model-high-lc}} for a low-complexity task — consider {{model-low-lc}}" or
"Agent E-tests failed twice with {{model-low-lc}} — upgrade to {{model-mid-lc}}".
...
3. **Compute trends over time**: campaign size, cost per agent, success rate,
   model-selection aggressiveness (share of {{model-low-lc}}).
...
Usage Stats — Trends (last 5 campaigns)
  ...
  Trends: success improving (88% -> 100%); cost/agent decreasing
          ($0.40 -> $0.31, -22%); {{Model-low}} usage increasing (25% -> 50%)

  Recommendations:
    - Move more medium-complexity tasks to {{model-low-lc}} for further savings
    - Current tiered strategy saves ~45% vs all-{{model-high-lc}}
```
...
# hooks_log = ".{{provider-lc}}/hooks/logs/hooks-log.jsonl"
# history_sessions = 10
...
|-------|----------------------|
| `{{cmd}}planner` | Budget awareness and trend data inform agent model selection |
| `{{cmd}}manager` | Cost tracking per campaign validates tiered-model savings; include efficiency in `{{cmd}}manager verify` |
| Observation log | Budget alerts and efficiency findings can be recorded in `data/observations.jsonl` |
| `{{cmd}}ship` | Include session summary in commit messages or PR descriptions |
...
- When comparing models, normalize by complexity tier (don't compare
  {{model-low-lc}} on easy tasks against {{model-high-lc}} on hard tasks)
{{#claude}}
