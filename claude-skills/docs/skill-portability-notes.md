...
Current portability status for the exported `claude-skills` package.
...
- `.claude/skills/planning-contract.md`
...
- Spec templates use `[commands]` from `.claude/skills/project.toml`.
...
- Runtime docs may describe `.claude/skills/...` paths when talking about the
...
- `plan-009` and `plan-011` illustrate runtime plan flows after the package has
  been vendored into a target repo, so `.claude/skills/...` paths are expected
  there.
...
The generated `.claude/skills/project.toml` should contain rendered values, not
