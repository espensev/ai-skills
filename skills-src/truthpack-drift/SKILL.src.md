...
# Truthpack Drift {{dash}} Truth Store Verification
...
**Config:** `.{{provider-lc}}/skills/project.toml` {{dash}} truth stores in `[truthpack-drift]`
...
|---------|-------|---------|
| `check` | `{{cmd}}truthpack-drift` or `{{cmd}}truthpack-drift check` | Full drift scan across all truth files |
| `file` | `{{cmd}}truthpack-drift file routes.json` | Check a single truth file |
| `regen` | `{{cmd}}truthpack-drift regen` | Suggest updates for drifted files |
...
## Command: `check` {{dash}} Full Drift Scan
...
## Command: `file` {{dash}} Single File Check

Same validation but scoped to one truth file. Faster for targeted checks.

Usage: `{{cmd}}truthpack-drift file routes.json`

---

## Command: `regen` {{dash}} Regeneration Suggestions
...
Does NOT auto-modify truth files {{dash}} only suggests. Truth stores are
human-curated artifacts; automated changes need review.

### Report

```
Truthpack Drift {{dash}} Regen Suggestions
...
|-------|--------------------------|
| `{{cmd}}manager verify` | Include truthpack validation in campaign exit criteria |
| `{{cmd}}schema-validator` | Parallel validator {{dash}} both check drift from different angles |
| `{{cmd}}ship` | Quick drift check before commit catches stale references |
| `{{cmd}}docs-sync` | Parallel validator {{dash}} truthpack checks truth-to-code; docs-sync checks docs |
| `{{cmd}}observer` | Record `drift` observations for truth store divergence |
...
- The codebase is always right {{dash}} truth files must reflect code
- Never auto-modify truth files {{dash}} only suggest changes
