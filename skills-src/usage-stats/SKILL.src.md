...
{{/claude}}
{{#codex}}
Codex transcripts do not expose exact per-message token counts, so when
{{/codex}}
...
`{{cmd}}manager` orchestration). Complements `{{cmd}}campaign-health` (state
...
|-------|----------------------|
| `{{cmd}}campaign-health` | Token costs and agent performance feed campaign efficiency metrics |
...
| `{{cmd}}observer` | Budget alerts and efficiency findings can be recorded as observations |
