...
around {{dash}} they pop windows into the user's real session.
...
needs a clean profile {{dash}} each `-Port` gets its own state dir.
...
must be started with `--browser-url http://127.0.0.1:9000` {{dash}} if your MCP
