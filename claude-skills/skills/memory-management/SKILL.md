...
| `compress` | `/memory-management compress` | Clean the store: dedupe against always-loaded context, delete tombstones, merge same-scenario files, cut filler, and fix or drop false and irrelevant entries — `references/compress.md` |
...
`/memory-management` with no subcommand runs the full hygiene pass:
