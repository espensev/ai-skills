...
Your output is a complete plan that can be handed off to `{{cmd}}manager run` for execution.

**You do NOT launch agents or execute work. You only design and document plans.**

**Planning contract:** Read `.{{provider-lc}}/skills/planning-contract.md` first — it
...
```
{{cmd}}planner <description of what to build or change>
{{cmd}}planner --mode refactor <description of refactor goal>
{{cmd}}planner --mode refactor next
```

Examples:

```
{{cmd}}planner Add real-time WebSocket push for dashboard auto-refresh
{{cmd}}planner Add a new provider for external API tracking
{{cmd}}planner --mode refactor Extract collector.py storage layer into collector_services.py
{{cmd}}planner --mode refactor Migrate frontend transport from HTTP to host bridge
{{cmd}}planner --mode refactor next
```
...
1. Read `.{{provider-lc}}/skills/project.toml` — project config
...
launches nothing — actually launching agents stays with `{{cmd}}manager run`,
...
markdown file is the human-readable derived campaign record. `{{cmd}}manager verify`
...
To execute:
  1. {{cmd}}manager run ready       ← launches agents, auto-advances all groups
  2. {{cmd}}manager merge            ← merges worktrees into main
  3. {{cmd}}manager verify           ← build, test, readiness check

To review specs first: read agents/agent-{letter}-{name}.md
```

The JSON plan file is the authoritative source for exit criteria. `{{cmd}}manager verify`
...
Read `.{{provider-lc}}/skills/project.toml` and the conventions file for project-specific
...
2. **Report the gap** to the user with a specific recommendation:
   ...
   Recommended: {{cmd}}discover {targeted research question}
   ```
3. After discovery completes, the user re-invokes `{{cmd}}planner` with the same
...
When the user invokes `{{cmd}}planner --mode refactor` or describes a refactor,
