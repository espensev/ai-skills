...
# Browser Control {{dash}} DevHome CDP Routing
...
deliberately disabled and must not be re-enabled or worked around {{dash}} they
...
profile {{dash}} each `devbrowser -Port` gets its own state dir.
...
If an endpoint probe fails, run `devbrowser` first {{dash}} do not start Opera or
Chrome yourself.
...
**Option A {{dash}} chrome-devtools-mcp (rich toolset).** Preferred for
...
**Option B {{dash}} direct CDP via `cdp.mjs`.** Zero-dependency Node script
...
**Option C {{dash}} raw HTTP for quick checks.**
...
- `devbrowser` mutations are identity-gated to this machine (snd-desk); if it
  refuses, stop and report {{dash}} do not bypass via manual process launch.
